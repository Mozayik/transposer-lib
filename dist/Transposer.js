...
class Transposer {
  ...
  */
    ...
    song.split(/\r?\n/gm).forEach(line => {
      ...
      if (chords.length > 0) {
        ...
        chords.forEach((chord, index) => {
          ...
          parts.push([progressionIndex, line.slice(offsetAfterChord, index === chords.length - 1 ? undefined : chords[index + 1].offset)]);
        });
        lines.push({
          type: "chords",
          parts
      ...
      if (lastLine && lastLine.type === "chords") {
        const lastLineLength = lastLine.parts.reduce((sum, part) => sum + (part[0] !== null ? progression[part[0]].length : 0) + part[1].length, 0);
        line = line.padEnd(Math.max(line.length, lastLineLength));
        const parts = [];
        let beginIndex = 0;
        let endIndex = 0; // Chop line up into same number of parts as the chords array in the last line
  ...
  stringify(keyName) {
    ...
    newLines = newLines.map((line, lineIndex) => line.type === "lyrics" ? line.parts.map((part, partIndex) => part.padEnd(Math.max(part.length, newLines[lineIndex - 1].parts[partIndex].length))).join("").trimEnd() : line);
    newLines = newLines.map(line => line.type === "chords" ? line.parts.join("").trimEnd() : line).join("\n");
    return newLines;
  }
  ...
  static guessKey(progression) {
    ...
    if (!likelyKeys.VI && !likelyKeys.iiVI && !likelyKeys.IVVI && !likelyKeys.IVI && !likelyKeys.Vvi && !likelyKeys.IV && !likelyKeys.viIV && !likelyKeys.iiivi) {
